...
```{targets data-makeup-3, include=FALSE}
list(
  tar_target(config_where_makeup3, config::get("where", file = file_config, config = "makeup3")),
  tarchetypes::tar_age(
    users_project_progress_makeup3,
    pickup(query_tmpl_progress, config_where_makeup3),
    age = as.difftime(3, units = "hours")
  )
)
```
...
```{targets check-makeup-3, include=FALSE}
list(
  tar_target(
    makeup_lang_leftover,
    makeup_lang |> 
      anti_join(
        filter(users_project_progress_makeup3, project_progress == 100),
        by = "user_id"
      )
  ),
  tar_target(
    file_user_makeup_lang_leftover,
    makeup_lang_leftover |>
      left_join(users, by = "user_id")  |>
      select(
        姓名 = user_name,
      ...
      writexl::write_xlsx("makeup/users_lang_leftover.xlsx"),
    format = "file"
  )
)
```
